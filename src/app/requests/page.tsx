...
import { ArrowUpDown, AlertCircle, CheckCircle2Icon, PlusCircle, Save, LogIn, ClipboardList } from 'lucide-react';
...
const REQUESTS_MODEL_HEADERS = ['Richiedente', 'Prodotto', 'Quantita', 'Budget', 'Anno'];
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-background">
        <Card className="w-full max-w-md shadow-2xl">
            <CardHeader className="text-center">
                ...
                <CardDescription className="text-muted-foreground">Please sign in to manage Request data.</CardDescription>
            </CardHeader>
            <CardContent>
...
const RequestsManagementPage: NextPage = () => {
  ...
  const loadDataFromDB = useCallback(async () => {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        setTableData(data);
        setTableHeaders(getAllKeys(data).length > 0 ? getAllKeys(data) : REQUESTS_MODEL_HEADERS);
        if (data.length > 0) {
          setSuccessMessage("Request data successfully loaded from database.");
        } else {
          setSuccessMessage("Request database is empty. Add a new row to start.");
        }
      } else {
        throw new Error("Invalid data format received from server.");
      }
    } catch (err) {
      setError(`Failed to load Request data from database: ${err instanceof Error ? err.message : String(err)}`);
      setTableData([]);
      setTableHeaders(REQUESTS_MODEL_HEADERS);
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const saveDataToDB = useCallback(async () => {
    ...
    try {
      ...
      setSuccessMessage("Request data successfully saved to database.");
      loadDataFromDB();
    } catch (err) {
      setError(`Failed to save Request data to database: ${err instanceof Error ? err.message : String(err)}`);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSaveEdit = useCallback(() => {
    ...
    if (headerKey === 'Anno' || headerKey === 'Quantita') {
      const num = parseInt(currentEditValue, 10);
      parsedNewValue = isNaN(num) ? 0 : num;
    } else if (headerKey === 'Budget') {
  ...
  const handleAddRow = useCallback(() => {
    ...
    REQUESTS_MODEL_HEADERS.forEach(header => {
        switch(header) {
            case 'Quantita':
            case 'Budget':
            case 'Anno':
    ...
    if (currentHeaders.length === 0 || !REQUESTS_MODEL_HEADERS.every(h => currentHeaders.includes(h))) {
        const headersWithId = (tableData && tableData.length > 0 && tableData[0].hasOwnProperty('id'));
        currentHeaders = [...REQUESTS_MODEL_HEADERS];
        if (headersWithId) {
    ...
    setSuccessMessage("New request row added. Click cells to edit. Save to persist changes to DB.");
    setError(null);
  ...
  return (
    <div className="container mx-auto p-0">
      <div className="text-left mb-8">
            ...
            <p className="text-muted-foreground mt-2">View, edit, add, and save user request data.</p>
      </div>
        ...
        <Card className="shadow-lg rounded-xl">
          ...
          <CardContent>
            {tableData === null && isLoading ? (
              ...
              <p className="text-muted-foreground text-center py-8">No request data to display. Try adding a row.</p>
                      ...
                      {tableHeaders.map((headerKey) => (
                        ...
                        >
                          {editingCell && editingCell.rowIndex === rowIndex && editingCell.headerKey === headerKey ? (
                              <Input
                                type={headerKey === 'Quantita' || headerKey === 'Anno' || headerKey === 'Budget' ? 'number' : 'text'}
